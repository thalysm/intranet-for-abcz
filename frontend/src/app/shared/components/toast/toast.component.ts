        ...
        <div class="p-3">
          <div class="flex items-center">
            <div class="flex-shrink-0">
              <svg *ngIf="toast.type === 'success'" class="h-4 w-4 text-green-400" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
              </svg>
              
              <svg *ngIf="toast.type === 'error'" class="h-4 w-4 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
              </svg>
              
              <svg *ngIf="toast.type === 'warning'" class="h-4 w-4 text-amber-400" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
              </svg>
              
              <svg *ngIf="toast.type === 'info'" class="h-4 w-4 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="ml-3 flex-shrink-0 flex">
              <button (click)="removeToast(toast.id)" 
                      class="inline-flex text-stone-400 hover:text-stone-500 focus:outline-none focus:text-stone-500 transition ease-in-out duration-150">
...
export class ToastComponent implements OnInit, OnDestroy {
  ...
  getToastClasses(type: Toast['type']): string {
    ...
    switch (type) {
      ...
      case 'warning':
        return `${baseClasses} border-amber-400`;
      case 'info':
